...
export const InfoItem: React.FC<InfoItemProps> = ({
  ...
  const containerAlignClass = text ? 'items-start' : 'items-center';
  ...
  const textStyleClass = variant === 'compact'
    ...
    : 'text-slate-500';
  return (
    <div className={cn("flex gap-3 cursor-pointer group", containerAlignClass)}>
      ...
      <div className="flex-1 min-w-0 text-left">
        <div className={headerLayoutClass}>
          ...
          {time && <span className="text-[10px] font-medium text-slate-400 shrink-0">
            {time}
          </span>
          }
          {isUnread && (
        ...
        {text && <p className={cn("text-[12px] text-slate-500 truncate leading-tight mt-0.5", textStyleClass)}>
          {text}
        </p>
        }
      </div>
